...
#Objective function
@objective(m, Min, sum((pl[l,t]*Sbase)^2*r[l] for l=1:NL,t=1:T) + sum(a[g]*p[g,t]^2 + c[g]*p[g,t]*Sbase for g=1:NG,t=1:T) + sum(VOLL*Ls[i,t]*Sbase for i=1:NI,t=1:T) +
sum(VOLW*pwc[i,t]*Sbase for i=1:NI,t=1:T));

solve(m)

OV = getobjectivevalue(m);
